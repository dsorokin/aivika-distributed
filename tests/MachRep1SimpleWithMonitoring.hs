...
runModel timeServerId =
  do DP.say "Started simulating..."
     let ps = defaultDIOParams { dioLoggingPriority = NOTICE,
         ...
         env = defaultDIOEnv { dioSimulationMonitoringAction = Just simulationMonitor }
         m =
           do registerDIO
...
simulationMonitor :: Show a => a -> DP.Process ()
simulationMonitor a =
  liftIO $
  do print ""
     print a
...
master = \backend nodes ->
  do liftIO . putStrLn $ "Slaves: " ++ show nodes
     let timeServerParams = defaultTimeServerParams { tsLoggingPriority = WARNING,
         ...
         env = defaultTimeServerEnv { tsSimulationMonitoringAction = Just simulationMonitor }
