...
-- |
-- Module     : Simulation.Aivika.Distributed.Optimistic.Internal.Dequeue.Unboxed
...
newDequeue :: MV.Unbox a => IO (Dequeue a)
{-# INLINABLE newDequeue #-}
newDequeue = 
  do array <- MV.new 4
...
-- | Copy the dequeue.
copyDequeue :: MV.Unbox a => Dequeue a -> IO (Dequeue a)
{-# INLINABLE copyDequeue #-}
copyDequeue dequeue =
...
dequeueEnsureCapacity :: MV.Unbox a => Dequeue a -> Int -> IO ()
{-# INLINABLE dequeueEnsureCapacity #-}
...
appendDequeue :: MV.Unbox a => Dequeue a -> a -> IO ()          
{-# INLINABLE appendDequeue #-}
...
readDequeue :: MV.Unbox a => Dequeue a -> Int -> IO a
{-# INLINE readDequeue #-}
...
writeDequeue :: MV.Unbox a => Dequeue a -> Int -> a -> IO ()
{-# INLINE writeDequeue #-}
...
dequeueBinarySearch' :: (MV.Unbox a, Ord a) => Dequeue a -> a -> Int -> Int -> IO Int
{-# INLINABLE dequeueBinarySearch' #-}
...
dequeueBinarySearch :: (MV.Unbox a, Ord a) => Dequeue a -> a -> IO Int
{-# INLINABLE dequeueBinarySearch #-}
...
freezeDequeue :: MV.Unbox a => Dequeue a -> IO (V.Vector a)
{-# INLINABLE freezeDequeue #-}
...
dequeueLast :: MV.Unbox a => Dequeue a -> IO a
{-# INLINE dequeueLast #-}
dequeueLast dequeue =
  do array <- readIORef (dequeueArrayRef dequeue)
     end   <- readIORef (dequeueEndRef dequeue)
